...
export const defaultInputTheme: InputTheme = {
  base: [
    'flex relative flex-row items-center flex-nowrap box-border transition-colors rounded-xs bg-panel border border-panel-accent text-text-primary hover:border-primary-hover'
  ].join(' '),
  focused: '',
  input:
    'flex-1 font-normal font-sans bg-transparent border-0 p-0 m-0 disabled:pointer-events-none outline-hidden px-0.5 disabled:cursor-not-allowed disabled:text-text-secondary/50 placeholder-accent',
  inline: 'bg-transparent border-0 outline-hidden',
  disabled:
    'cursor-not-allowed opacity-40 hover:border-panel-accent pointer-events-none',
  fullWidth: 'w-full',
  error: 'border-error',
  sizes: {
